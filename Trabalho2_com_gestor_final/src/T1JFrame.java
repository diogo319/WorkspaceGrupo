...
public class T1JFrame extends JFrame {
	...
	public void run() {
		pc.enviarMensagem(Mensagem.GUI);
		while(run) {
			...
			switch(msg) {
				...
				case VAGUEAR_TERMINADO:
					if(!gestorCheckBox.isEnabled()) {
						pc.enviarMensagem(Mensagem.GUI);
						vaguearCheckBox.setSelected(false);
						onOffRadioButton.setEnabled(true);
						actualizarCheckBoxes(true);
						myPrint("Vaguear terminado");
					} else {
						//Esperar que o gestor veja a mensagem
						try {
							Thread.sleep(500);
						} catch (InterruptedException e) {
							// TODO Auto-generated catch block
				...
				case EVITAR_TERMINADO:
					...
					break;
				case GESTOR_TERMINADO:
					pc.enviarMensagem(Mensagem.GUI);
					gestorCheckBox.setSelected(false);
					onOffRadioButton.setEnabled(true);
					actualizarCheckBoxes(true);
					myPrint("Gestor e sub-processos terminado");
					break;
			}
		}
	}
	...
	public T1JFrame() {
		...
		debugCheckBox.setBounds(6, 293, 67, 23);
		...
		consolaJTextField.setBounds(69, 323, 410, 20);
		...
		consolaJLabel.setBounds(6, 326, 473, 14);
		...
		addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent arg0) {
				...
				pc.enviarMensagem(Mensagem.TERMINAR);
				pc.fecharCanal();
				System.exit(1);
			}
		});
			...
			public void actionPerformed(ActionEvent e) {
				if(vaguearCheckBox.isSelected()) {
					...
					if(p != null) {
						...
						onOffRadioButton.setEnabled(false);
						vaguearCheckBox.setSelected(true);
			...
			public void actionPerformed(ActionEvent e) {
				if(evitarCheckBox.isSelected()) {
					...
					if(p != null) {
						...
						myPrint("EvitarObstaculos lançado");
						onOffRadioButton.setEnabled(false);
						evitarCheckBox.setSelected(true);
		...
		gestorCheckBox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(gestorCheckBox.isSelected()) {
					Process p = null;
					try {
						p = Runtime.getRuntime().exec("java -jar D:\\ISEL\\2016-2017\\FSO\\Gestor.jar");
					} catch (IOException e1) {
						e1.printStackTrace();
					}
					
					if(p != null) {
						myPrint("Gestor lançado");
						onOffRadioButton.setEnabled(false);
